    ...
    def get(self,request):
        try:
            ...
            if request.GET.get('id'):
                ...
                lst_agent_data = UserDetails.objects.filter(int_type = 2,user__is_active = True).values('user__first_name','user__last_name','phone_number','alt_phone_number','user__email','address','user')
                return Response({'data':lst_agent_data},status = HTTP_200_OK)
        ...
        try:
            with transaction.atomic():
                if request.data.get('id') and User.objects.filter(id = request.data.get('id')).exists():
                    ...
                    return Response({"Message": "Agent deleted successfully"}, status=HTTP_200_OK)
                else:
                    raise Exception('Agent not found')
        except Exception as e:
            return Response({"Message":str(e)},status = HTTP_500_INTERNAL_SERVER_ERROR)
...
# class AgentAPIViewSet(viewsets.ViewSet):
